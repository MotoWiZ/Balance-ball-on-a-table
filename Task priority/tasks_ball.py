...
sp_max = os.sched_get_priority_max(os.SCHED_RR)
prio_value = 99
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if time.time() - prt_time > 1:
          print("get_image time: ", "%.5f" % (time.time() - start_time))
        img = original.copy()
        ...
        if time.time() - prt_time > 1:
            print("Windows time: ", "%.5f" % (time.time() - windows_time))        
        # Press 'q' to quit
        ...
        if time.time() - t > 2:
            ...
            t = time.time()
        if time.time() - prt_time > 1:
            print("Whil_loop time: ", "%.5f" % (time.time() - start_time))
            prt_time = time.time()
